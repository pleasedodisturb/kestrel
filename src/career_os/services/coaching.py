...
def _build_skill_gap_suggestions(
    ...
    for skill_key, data in gap_data.items():
        ...
        difficulty = (
            "low" if distance == 1 else "medium" if distance == 2 else "high"
        )
...
def _build_goal_suggestions(db: Session, profile_id: int) -> list[dict]:
    ...
    goals = (
        db.query(Goal)
        .filter(Goal.profile_id == profile_id, Goal.status == "active")
        .all()
    )
    ...
    active_apps = sum(
        1 for a in applications if a.status in ("applied", "interviewing", "offer")
    )
    ...
    in_progress_learning = sum(
        1 for lr in learning_resources if lr.status == "in_progress"
    )
...
def get_coaching_suggestions(
    ...
    sorted_suggestions = sorted(
        seen.values(), key=lambda s: s["priority_score"], reverse=True
    )
