...
async def openrouter_credits(db: Annotated[Session, Depends(get_db)]) -> dict:
    """Check OpenRouter credit balance using the stored API key."""
    api_key = _get_stored_api_key(db)
    if not api_key:
        raise HTTPException(
            status_code=404,
            detail="No OpenRouter API key configured. Complete setup first.",
        )
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"Credits check failed ({exc.response.status_code}).",
        ) from exc
    except httpx.RequestError as exc:
        raise HTTPException(
            status_code=502,
            detail="Could not reach OpenRouter to check credits.",
        ) from exc
