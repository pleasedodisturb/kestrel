...
Connects to the Groq API (https://api.groq.com) for fast open-source
...
class GroqProvider(AIProvider):
    """AI provider backed by the Groq API (OpenAI-compatible)."""
    ...
    def privacy_tier(self) -> str:
        """Groq does not train on API data — green tier."""
        return "green"
        ...
        for attempt in range(1, max_retries + 2):
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                ...
                if response.status_code in (402, 429):
                    ...
                    raise ProviderQuotaError("groq", response.status_code, detail)
...
def _extract_error_detail(response: httpx.Response) -> str:
    """Best-effort extraction of error message from a Groq error response."""
    try:
        body = response.json()
