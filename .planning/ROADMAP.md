# Roadmap: Kestrel

## Milestones

- ✅ **roadmap-m1 (Public Roadmap)** — Phases 2-5 (shipped 2026-05-07) — see [archive](milestones/roadmap-m1-ROADMAP.md)

## Phases

<details>
<summary>✅ roadmap-m1 (Public Roadmap) (Phases 1-5) — SHIPPED 2026-05-07</summary>

- [ ] Phase 1: Feature Inventory (0/1 plans) — _deferred to v1.1_
- [x] Phase 2: Roadmap Foundation (2/2 plans) — completed 2026-04-25
- [x] Phase 3: Forward Vision (2/2 plans) — completed 2026-04-26
- [x] Phase 4: Milestone Deep Dives (2/2 plans) — completed 2026-04-27
- [x] Phase 5: Contributor Experience (2/2 plans) — completed 2026-05-07

Full archive: `.planning/milestones/roadmap-m1-ROADMAP.md`
Audit: `.planning/milestones/roadmap-m1-MILESTONE-AUDIT.md`

</details>

### 📋 v1.1 Next Milestone (Planned)

To be defined via `/gsd-new-milestone`. Open candidates carrying over from v1.0:

- Feature Inventory (INV-01..08) — deferred from v1.0; only worth picking up if a unified standalone inventory.md is wanted (much of the intent is already covered by deep dives)

## Progress

| Phase | Milestone | Plans Complete | Status | Completed |
|-------|-----------|----------------|--------|-----------|
| 1. Feature Inventory | v1.0 | 0/1 | Deferred to v1.1 | — |
| 2. Roadmap Foundation | v1.0 | 2/2 | Complete | 2026-04-25 |
| 3. Forward Vision | v1.0 | 2/2 | Complete | 2026-04-26 |
| 4. Milestone Deep Dives | v1.0 | 2/2 | Complete | 2026-04-27 |
| 5. Contributor Experience | v1.0 | 2/2 | Complete | 2026-05-07 |
