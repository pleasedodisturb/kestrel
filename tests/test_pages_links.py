...
class TestDocumentationLinks:
    ...
    def test_all_readme_doc_links_resolve(self) -> None:
        ...
        md_links = [(text, url) for text, url in links if url.endswith('.md') and not url.startswith('http')]
    ...
    def test_all_doc_files_have_front_matter(self) -> None:
        ...
        for f in doc_files:
            if f.name.startswith(skip_prefixes):
                continue
            content = f.read_text()
            assert content.startswith("---"), f"{f.name} missing Jekyll front matter (should start with ---)"
    ...
    def test_no_pricing_strategy_in_repo(self) -> None:
        ...
        assert not (ROOT / "docs" / "pricing-strategy.md").exists(), "pricing-strategy.md should not be in public repo"
