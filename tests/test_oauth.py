...
class TestCreditsEndpoint:
    """Tests for GET /api/auth/openrouter/credits."""

    def test_no_key_returns_404(self, db_client):
        """Returns 404 when no API key is configured."""
        import career_os.api.oauth as oauth_mod

        oauth_mod._runtime_api_key = ""
        with patch("career_os.api.oauth.settings") as mock_settings:
            mock_settings.openrouter_api_key = ""
            resp = db_client.get("/api/auth/openrouter/credits")

        assert resp.status_code == 404
        assert "No OpenRouter API key" in resp.json()["detail"]

    def test_success_returns_balance(self, db_client):
        """Returns credit balance when key is configured."""
        import career_os.api.oauth as oauth_mod

        oauth_mod._runtime_api_key = "sk-or-test-key"  # noqa: S105

        mock_resp = httpx.Response(
            200,
            json={"data": {"total_credits": 25.0, "total_usage": 3.47}},
        ...
        assert resp.status_code == 200
        data = resp.json()
        assert data["total_credits"] == 25.0
        assert data["total_usage"] == 3.47
        assert data["balance"] == 21.53
        assert data["needs_deposit"] is False

    def test_low_balance_flags_deposit(self, db_client):
        """needs_deposit is True when balance < $1."""
        import career_os.api.oauth as oauth_mod
        ...
        assert resp.status_code == 200
        assert resp.json()["needs_deposit"] is True

    def test_openrouter_http_error_returns_status(self, db_client):
        """OpenRouter HTTP error is forwarded."""
        import career_os.api.oauth as oauth_mod
        ...
        with patch("career_os.api.oauth.httpx.AsyncClient") as mock_cls:
            ...
            mock_resp.raise_for_status = lambda: (_ for _ in ()).throw(
                httpx.HTTPStatusError("", request=mock_resp.request, response=mock_resp)
            )
            mock_cls.return_value = mock_client
    ...
    def test_network_error_returns_502(self, db_client):
        ...
        assert resp.status_code == 502
        assert "Could not reach OpenRouter" in resp.json()["detail"]
