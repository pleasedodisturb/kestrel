...
SCORED_DATA_PATH = Path(__file__).resolve().parent.parent / "tracking" / "scraped_scored_2026-03-27.json"
    ...
    def test_prefilter_rejects_significant_portion(self, raw_jobs):
        ...
        for j in raw_jobs:
            skip, _, _ = pre_filter_job(
                ...
                j.get("location", ""), bool(j.get("remote", False))
            )
            if skip:
                skipped += 1
    ...
    def test_generate_report(self, raw_jobs, scored_jobs):
        ...
        for j in raw_jobs:
            ...
            results.append({
                ...
                "old_score": old_score,
                "new_score": new_score,
                "filter_action": "REJECTED" if should_skip else (f"CAPPED at {cap}" if cap else "PASS"),
        ...
        good_keywords = ["product manager", "tpm", "devrel", "developer", "engineer", "ai", "founding"]
        for r in results:
            if r["filter_action"] == "PASS" and any(kw in r["title"].lower() for kw in good_keywords):
                lines.append(
